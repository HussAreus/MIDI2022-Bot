...
def check_answer(uid, answer):
    ...
    with open("quests.json", "r") as f:
        ...
        if answer in data.keys():
            ...
            if guild.mapTokens and quest.map in guild.mapTokens:
...
def load_all(uid):
    ...
    for wmap in map_dict.keys():
        i += 1
        if guild and guild.mapTokens and wmap in guild.mapTokens:
            embed.add_field(name=map_dict[wmap].location, value="_ _", inline=False)
        else:
...
map_dict = {
    "backdoor": WorldMap({
        ...
        [
            Quest("Petro Cvirkos aikštė", "https://drive.google.com/file/d/1CZbhaypo4420ee5RXrXxXeJcIjpRi1f9/view?usp=sharing", "Kur reikėtų padėti baltą figūrą, kad būtų skelbiamas Matas? https://drive.google.com/file/d/15-It7cJoKBXKXt6giqGeW8dzGHMYrBXr/view?usp=sharing", "h5", 20, 20, "backdoor"),
            Quest("Frank Zappa", "https://drive.google.com/file/d/1nCKpluYvUobjZtmZllJM6RGvUNeBaz2W/view?usp=sharing", "Kiek paukščių matote ant sienos?", "8", 20, 10, "backdoor"),
            Quest("Reformatų skveras", "Ieškokite riedulio su šiuo tekstu: „Čia, buvusiose kapinėse, bus pastatytas paminklas XVI a. Lietuvos reformacijos pradininkams.“", "Kad jau radai tokį masyvų riedulį, nusipelnei ir masyvių taškų", "SHUT THE FUCK UP", 10, 10, "backdoor"),
            ...
            Quest("MO muziejus", "x = -4,5\ny = |8x + 32| - 1, x = [-4,5; -3,5]\ny = |8x + 24| - 1, x = [-3,5; -2,5]\ny = |8x + 16| - 1, x = [-2,5; -1,5]\nx = -1,5\ny^2 + x^2 = 1", "Kokia tai mįslė https://drive.google.com/file/d/1DC7HTVeeGeD7f3_h1TvJXM6R238D4kIC/view?usp=sharing", "Verkia duona tinginio valgoma", 30, 20, "backdoor")
        ]),
    ...
    "machine city": WorldMap({
        ...
        [
            ...
            Quest("Berniukas su kaliošu", "„Ką galima padaryti dėl meilės? Galbūt net suvalgyti kaliošą“ - Romanas Garis", "Nekankinsime, valgyti nieko nereikalausime, pabūsime geraširdžiais ir dovanosime Jums  taškų. :)", "NE NEDOVANOSIM", 15, 10, "machine city"),
            Quest("Provianto kvartalas", "Buvęs mažas ir gilus gyvybės šaltinis tarp naujoviškų dangoraižių. 54,6793855, 25,2582647", "Aukščiausias dangoraižis siekia net 828 metrų aukštį, o kokį gylį siekia mūsų pastatas.", "116.95", 15, 30, "machine city"),
    ...
    "capital city": WorldMap({
        ...
        [
            ...
            Quest("Katedra ir Valdovų Rūmai", "Jeigu netyčia pasiklystumėte matricoje ir nusikeltumėte atgal į praeitį einant šiuo koridoriumi galėtumėte prasilenkti su krikščionių dvasininkais ir Lietuvos kunigaikščiais. ", "Fizine užduotis", "striksintis katinas", 10, 20, "capital city"),
            Quest("Seniausias skelbimų stulpas", "https://drive.google.com/file/d/1m-UJxhtPuE-oCm_8_itW_6PTKQ3MDP9g/view?usp=sharing", "Kas yra X?", "seniausias skelbimų stulpas", 0, 40, "capital city")
        ]),
